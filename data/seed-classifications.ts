...
if (!CLASSIFICATION_TABLE && !VENDOR_TABLE) {
  console.error(
    'Error: At least one of CLASSIFICATION_TABLE or VENDOR_TABLE environment variables is required.',
  )
  console.error(
    'Usage: AWS_REGION=<region> CLASSIFICATION_TABLE=<name> VENDOR_TABLE=<name> npx ts-node data/seed-classifications.ts',
  )
  process.exit(1)
}
...
const seed = async () => {
  if (CLASSIFICATION_TABLE) {
    ...
    console.log(
      `Seeded ${classifications.length} classification configs into ${CLASSIFICATION_TABLE}.`,
    )
  }
